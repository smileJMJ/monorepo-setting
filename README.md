...
- 공통 유틸 / 코드 컨벤션 설정(eslint, prettier, tsconfig.json 등) 등은 root에 정의한다.
  - root에서 내부 폴더로 정의하여, 다른 워크스페이스의 import 구문에서 가져온다.  
    → 공통 유틸, 코드 컨벤션 관련 설정들을 해당 모노레포에서만 사용할 때 / react, vue 등 프레임워크를 워크스페이스에서 공통으로 사용할 때 괜찮을 듯??
  - 또는, 유틸, 컨벤션 관련 설정들도 워크스페이스로 정의한 후 다른 워크스페이스들에서 패키지 설치(`workspace:*`) 로 가져온다. (turborepo/kichen-sink 참고)
    → root에서 node_modules 갖지 않고, 각 워크스페이스별 독립적으로 사용하고자 할 때 더 좋을 듯?!
    → 워크스페이스별 다양한 프레임워크를 가질 때 더 좋은 구조일 듯
- 하위 워크스페이스들은 packages 폴더에 정의한다.
- 하위 워크스페이스들은 각각 다양한 환경을 구성할 수 있다. (react, vue, webpack, vite ...)
